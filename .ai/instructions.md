...
> **Note:** This guide is a verbatim copy of upstream ESPHome's `AGENTS.md` (https://github.com/esphome/esphome/blob/dev/AGENTS.md) and is kept for its coding standards, C++ style, component patterns, and embedded-systems guidance. Repo-layout specifics (e.g. `/docker`, `/script`, `platformio.ini`, `script/test_build_components`, the `dev` branch, PR templates) describe the upstream ESPHome repository and do **not** exist in this fork — see `CLAUDE.md` for this repo's actual layout and build commands.
